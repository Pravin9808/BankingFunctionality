...
public class BOBHLcalculator extends BaseClass {
	...
	public void axisPlCalAmortisation() throws InterruptedException, IOException, InvalidFormatException  {
		...
		XSSFSheet inpSheet= inpDataWkb.getSheetAt(1);
		...
		for(int i=0;i<wkb.getNumberOfSheets();i++){
            ...
            if(wkb.getSheetAt(i).getSheetName().equals("HomeLoanCalData")){
                wkb.removeSheetAt(i);
            }
        } 
		//wkb.removeSheetAt(1);
		wkb.createSheet("HomeLoanCalData");
		...
		for (int i = 2; i <=inpRowCount+1; i++) {
				XSSFRow currentrow = inpSheet.getRow(i);
				 ...
				 DataFormatter formatter = new DataFormatter();
					...
					XSSFSheet sheet1 = wkb.getSheet("HomeLoanCalData");
					int initialRow=sheet1.getLastRowNum()+2;
					
					
					
					driver.get("https://www.bankofbaroda.in/calculators/home-loan-emi-calculator");
					...
					WebElement rateofInterestTextBoxElement = driver.findElement(By.xpath("//em[normalize-space()='8.60']"));
					...
					WebElement loanTermsTextBoxElement = driver.findElement(By.xpath("//em[normalize-space()='240']"));
					...
					ArrayList<String> bobHlData=new ArrayList<String>();
					bobHlData.add(amount);
					bobHlData.add(rateOfInterest);
					bobHlData.add(loanTermsInMonths);		
					bobHlData.add(emi_Amt_BOB);
					bobHlData.add(intAmt_BOB);
					...
					row3.createCell(0).setCellValue("For BOB Bank");
					 ...
					 for (int a=0;a<bobHlData.size();a++) {	
						 
			            XSSFCell cell = row3.createCell(a+1);
			             cell.setCellType(CellType.STRING);
			             cell.setCellValue((String) bobHlData.get(a)); 
			         }	
					...
					driver.get("https://www.axisbank.com/retail/calculators/home-loan-emi-calculator");
					driver.manage().deleteAllCookies();
					Thread.sleep(6000);
					...
					driver.findElement(By.xpath("//input[@id='tenure']")).sendKeys(loanTermsInYears);
					...
					for (int l=1;l<=irowsAxis.size();l++) {
						...
						//System.out.println( icolsAxis.size());
						for (int p=1; p<icolsAxis.size();p++) { 
					...
					ArrayList<String> axisHlData=new ArrayList<String>();
					axisHlData.add(amount);
					axisHlData.add(rateOfInterest);
					axisHlData.add(loanTermsInYears);		
					axisHlData.add(emi_axis);
					axisHlData.add(int_paym_axis);
					int rowno4=rowno3+1;
					XSSFRow row4=sheet1.getRow(rowno4);					
					row4.createCell(0).setCellValue("For Axis Bank");
					
					 for (int c=0;c<axisHlData.size();c++) {	
						 
			            XSSFCell cell = row4.createCell(c+1);
			             cell.setCellType(CellType.STRING);
			             cell.setCellValue((String) axisHlData.get(c)); 
			         }	
